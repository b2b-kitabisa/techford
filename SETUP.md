...
2. Isi `SPREADSHEET_ID` di `src/00_Core/00_Config.gs` dengan ID Spreadsheet Anda. `ROOT_FOLDER_ID` (folder Shared Drive B2B tempat dokumen hasil generate seperti COR/Quotation disimpan) dan `COR_TEMPLATE_FILE_ID` (spreadsheet "Template COR") sudah diisi sesuai Shared Drive & template yang Anda berikan — pastikan akun yang jadi "Execute as" Web App (biasanya akun yang deploy, karena `executeAs: USER_DEPLOYING`) punya **akses edit** ke folder Shared Drive tersebut **dan** ke file "Template COR" itu sendiri. `QUOTATION_TEMPLATE_FILE_ID` juga sudah diisi dengan Google Doc master Quotation (berisi 4 section YKB-EN/YKB-ID/KAI-EN/KAI-ID) — belum dipakai untuk generate apa pun di Tahap 1, akan dipakai mulai Tahap 2, tapi pastikan akun yang sama juga punya akses edit ke file itu dari sekarang.
