...
1. Buat sheet berikut di Spreadsheet database platform ini:
   ...
   - `Quotation_Item` dengan header baris pertama: `Item_ID | Doc_ID | Category_Label | Category_Sort_Order | Item_Label | Item_Sort_Order | Value | Qty | Remarks_Detail`. Baris item layanan (tabel "Box Price") — satu kategori bisa punya beberapa baris item, `Value`/`Qty` boleh diisi di baris manapun (bukan cuma baris pertama), itu yang membedakan pola "satu harga untuk seluruh kategori" vs "harga per item".
